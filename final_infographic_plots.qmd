...
ggsave("largest_fires.png", plot = last_plot(),
       width = 12, height = 8)
```
...
map_change <- ggplot() +
  ...
  theme(legend.position = "bottom", # c(0.67, 0.78)
        plot.title = element_text(family = "Quicksand", face = "bold", size = 18,
                                   margin = margin(t = 0.25, r = 0, b = 0.25, l = 0, unit = "cm"),
                                  hjust = 0.5),
        plot.subtitle = element_text(family = "Quicksand", size = 15, hjust = 0.5),
        plot.background = element_rect(fill = "black"),
...
monthly <- calendar %>%   
  ...
  # add month labels
  geom_text(data=dates_axis, aes(x = x, y = y, label = mth), color ="white", inherit.aes = FALSE, size = 4) +
  ...
  scale_y_continuous(limits = c(0,3000000), expand = expansion(mult = c(0.3, 0))) +
  scale_x_continuous(breaks = seq(1,12,1), limits = c(1,13), labels = lubridate::month(seq(1,12,1), abbr=TRUE, label = TRUE))+
  # remove axes, etc.
  theme_void()+
  guides(color = "none", alpha = "none",
         fill = guide_colorbar(barwidth = 25, barheight = 0.75)) +
